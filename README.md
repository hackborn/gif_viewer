...
## purpose
There are many great GIF libraries available. The motivation for this one was primary pure curiosity about the format, although in looking through various implementations there wasn't really anything that hit my sweet spot: A couple of image format-independent classes I could drop into a project that had a minimal API for providing my own translation from whatever local bitmap storage class I happen to be using.

So this repo is really two things, an isolated folder with a few classes for loading GIF files (and hopefully writing at some point), as well as a simple viewer to verify everything's working.
...
* Everything in the main *src/* folder is app-specific to the test GIF viewer.
